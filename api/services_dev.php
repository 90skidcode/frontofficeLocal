...
// Others - Type,Amount,Gst,Total,RoomNO 
 
  $json = '{ "list_key": "CloseNightaudit","audit_date": "2021-09-23"}';
    ...
    if (is_null($data)) {
      echo json_encode(array("message"=>"Not a proper Json values","status_code"=>400));
       ...
       if(!empty($data['list_key'])){
           ...
           /* 
             $non_audit = array ('master_login','get_dashboard');
            
             if(!in_array($data['list_key'], $non_audit))
             {
                ...
                $row_audit = $result_audit->fetch_all(MYSQLI_ASSOC);
                 ...
                 $date2=date_create("2013-03-10");
                 $diff=date_diff($date1,$date2);
                 echo $diff->format("%a");
                echo date_diff($currentaudit_date,$current_date);
                echo json_encode(array("message"=>"Audit not Done","status_code"=>303));
                exit;
             }
             */
           ...
           switch($data['list_key'])
           {
            ...
            case "ReportsMaster":
                    $result = $select_class->get_all_reports($data,$conn,$where_values);
            break;
